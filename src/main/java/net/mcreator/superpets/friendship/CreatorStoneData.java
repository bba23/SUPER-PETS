...
public final class CreatorStoneData {
	...
	public static boolean isGeneratedCreatorStone(ItemStack stack) {
		return stack.getItem() == FriendshipRelicsRegistry.CREATOR_STONE && tag(stack).map(data -> data.getBooleanOr(GENERATED, false)).orElse(false);
	}
	...
	public static InteractionResult tryGenerateFromPet(ItemStack stack, Player player, LivingEntity target) {
		...
		String texturePath = FriendshipRelicsConfig.creatorStoneGeneratedAssetRoot + "/creator_stones/" + cacheKey + ".png";
		...
		stack.set(DataComponents.ITEM_MODEL, Identifier.fromNamespaceAndPath(SuperPetsMod.MODID, "creator_stone_" + petType));
	...
	public static ItemStack applyToTool(ItemStack tool, ItemStack creatorStone) {
		...
		String texturePath = FriendshipRelicsConfig.creatorStoneGeneratedAssetRoot + "/tools/" + cacheKey + "_" + toolKind + ".png";
		...
		result.set(DataComponents.ITEM_MODEL, Identifier.fromNamespaceAndPath(SuperPetsMod.MODID, "generated_tool_" + toolKind + "_" + petType));
	...
	private static String fallbackReason() {
		if (!FriendshipRelicsConfig.creatorStoneAiEnabled)
			return "AI disabled; deterministic fallback visual active.";
		if (FriendshipRelicsConfig.creatorStoneAiHelperCommand.isBlank())
